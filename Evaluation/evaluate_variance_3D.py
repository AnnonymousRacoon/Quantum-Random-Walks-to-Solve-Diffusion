...
from DiffusionProject.Algorithms.Walks import Boundary, QuantumWalk1D, QuantumWalk3D, OneWayBoundary
...
# initialise walk
walk = QuantumWalk3D(system_dims,initial_states,coin_class=HadamardCoin,boundaries=boundaries)
walk.step()

# begin experiment
print("Experimenting in 3 dimensions with on a {}*{} closed grid and a {}".format(2**n_qubits,2**n_qubits, walk.shift_coin._name))
for experiment_number in range(1,max_iterations+1,stepsize):

    experiment_name = "{}D_Walk_{}x{}_iteration_{}_{}".format(n_dims,2**n_qubits,2**n_qubits,experiment_number, walk.shift_coin._name)
